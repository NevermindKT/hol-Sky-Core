...
func _on_weather_changed():
	...
	var weather_state: WeatherState = WeatherManager.weather_state
	if weather_state == null:
		return

	if weather_state.rain == null:
		return

	if weather_state.rain.lightning_enabled:
...
func flash() -> void:
	...
	var weather_state: WeatherState = WeatherManager.weather_state
	if weather_state == null or weather_state.rain == null:
		flashing = false
		return
